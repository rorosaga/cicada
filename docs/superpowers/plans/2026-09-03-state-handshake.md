...
new53 = ("relates to G48, G49 (primer hook), G13, G50, context-passport roadmap. "
         "**Shipped 2026-09-03 (PR #88, `feat/state-handshake`):** `api/services/state_dictionary.py` writes `<bank>/_state.md` "
...
new75 = ("which G115 Phase 2 routes through the same gate. "
         "**Shipped 2026-09-03 (PR #88, `feat/state-handshake`):** `api/services/handshake.py` — ≤ 1,800 tokens, three variants "
...
2. `## ✅ Shipped` → **Provenance** line: append " · **G53 + G75 live state + handshake (2026-09-03, PR #88)** — `_state.md` cursor, `initialize.instructions`, `cicada_handshake`, `/state`, `/handshake`".
3. Wave C item `10. **G53 + G75** …` → strike (`~~…~~`) with "— shipped PR #88; open: SessionStart hook (G49/G76), Store fetch of `/state`".
