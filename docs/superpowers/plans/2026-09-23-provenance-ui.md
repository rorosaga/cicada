...
`rg -n "app PR #TBD" docs/goals/memory-evolution.md` → the one G118 status cell the orchestrator
...
4. **Merge hygiene.** Resolve `ContentView.swift` against Track S if it merged first (R-PU20 moves to
   the palette); take the union in `docs/goals/*` and `CLAUDE.md`; replace `PR #TBD` in the G118 row.
