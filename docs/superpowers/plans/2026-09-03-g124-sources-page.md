...
- [ ] **Step 1: CLAUDE.md sidebar paragraph (:635)** — replace the sentence `The sidebar is six rows — Graph, Clusters, Feed, Sleep, Inbox, Activity — reachable via ⌘1–6 …and Activity merges consumption and contributor attribution behind a segmented control with the origins strip.` with:
...
- [ ] **Step 2: CLAUDE.md API list** — after `GET  /contributors/commits…` (:706) add:
```
