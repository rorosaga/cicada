...
- Work ONLY in `/Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115` (branch `feat/inbox-phase1`). Every shell command is `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && <cmd>` with absolute paths (`zoxide` hijacks relative `cd`; ignore its stderr warning). No `grep --include=*.ext` (zsh globbing breaks it).
- NEVER read `/Users/rorosaga/Documents/roros_lab/cicada/memory` (any bank), `~/.cicada`, `~/Library/Safari` or `~/.claude/projects`. Test fixtures are synthetic (`alpha-project`, `bob-example`, `example.com`).
- Python tests: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest <files> -q -p no:cacheprovider`. Full suite `api/tests`: baseline has exactly 8 date-dependent failures in `test_calendar_registry.py` plus `test_agent_provenance.py::test_a_decay_only_change_lands_in_its_own_cicada_authored_commit` (order-dependent, pre-existing). Everything else must be green.
- Swift: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115/app/CicadaApp && swift build 2>&1 | tail -5` must succeed and `swift test 2>&1 | tail -20` must report 0 failures (SourceKit diagnostics naming OTHER worktrees are noise). NEVER run `make dev`, `make install-app`, `swift run`, or launch/kill the Cicada app — the owner's installed app is live.
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest api/tests/test_inbox_context.py api/tests/test_inbox_dedup.py -q -p no:cacheprovider`
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115/app/CicadaApp && swift test --filter VersionVectorTests 2>&1 | tail -8`
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest api/tests/test_inbox_questions.py api/tests/test_inbox_resolution_provenance.py api/tests/test_feedback_ledger.py -q -p no:cacheprovider 2>&1 | tail -12`
...
```bash
cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git add api/services/inbox_questions.py api/services/inbox_service.py api/tests/test_inbox_questions.py api/tests/test_inbox_resolution_provenance.py api/tests/test_feedback_ledger.py && git commit -m "feat(inbox): one question object for every kind — decay at read, (Recommended) from _verdict, remind_later→defer(7), observer setting (G115 p1)"
```
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest api/tests/test_mcp_inbox_questions.py -q -p no:cacheprovider 2>&1 | tail -8`
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest api/tests/test_mcp_inbox_questions.py api/tests/test_mcp_handshake.py api/tests/test_handshake.py api/tests/test_entity_read_events.py api/tests/test_mcp_recall_episode_fallback.py -q -p no:cacheprovider 2>&1 | tail -8`
...
```bash
cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git add mcp/server.py api/tests/test_mcp_inbox_questions.py && git commit -m "feat(mcp): render_question v2 — Cause line, (Recommended), entity_id header, skip hint; decay as a question (G115 p1)"
```
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115/app/CicadaApp && swift build --build-tests 2>&1 | grep -E "error:" | head -8`
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115/app/CicadaApp && swift build 2>&1 | tail -5 && swift test 2>&1 | tail -20`
...
Run: `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git diff --cached --stat; git diff -- CLAUDE.md docs/goals | grep -n -i -E "rodrigo|@|https?://" | grep -v "Rodrigo 20" | head`
...
```bash
cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git add CLAUDE.md docs/goals/memory-evolution.md docs/goals/TODO.md docs/superpowers/plans/2026-09-03-g115-inbox-phase1.md && git commit -m "docs(goals): G115 Phase 1 shipped — G97 delivered, G98 card rule, CLAUDE.md inbox section, handoff"
```
...
1. `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git status --porcelain -uall` → nothing untracked except scratch; no `memory/`, `logs/`, `*-report.md` staged in any commit (`git log --stat cf56ea2..HEAD | grep -E "memory/|logs/|report.md"` → empty).
2. `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && api/.venv/bin/python -m pytest api/tests -q -p no:cacheprovider 2>&1 | tail -15` → failures are EXACTLY the 8 `test_calendar_registry.py` date-dependent ones plus `test_agent_provenance.py::test_a_decay_only_change_lands_in_its_own_cicada_authored_commit`; nothing else.
3. `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115/app/CicadaApp && swift build 2>&1 | tail -5 && swift test 2>&1 | tail -20` → build OK, `0 failures`.
...
7. Ledger rail (G113): `cd /Users/rorosaga/Documents/roros_lab/cicada/.worktrees/g115 && git diff cf56ea2..HEAD -- api/services/inbox_service.py | grep "^+" | grep -nE "excerpt|conversation_title|entity_name|answer"` → no hit that lands inside `_emit_resolution`'s `refs` dict. Then read `_emit_resolution` end to end and confirm the only two added keys are `recommended_key` (a str) and `picked_recommended` (a bool) — no excerpt, label, title or name may reach the ledger, which lives outside the bank.
