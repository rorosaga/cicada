...
| G11 | **Image references / image artifacts as memories** | Support **image references** viewable **inside the app** — and let images-as-artifacts be saved as memories (e.g. an image attached to/embedded in an entity or claim, rendered inline like the transclusion layer renders embedded pages). Extends the inline-transclusion model (`![[…]]`) to image embeds (the D2 transclusion scope currently excludes images — revisit). Sources: chat-export images (e.g. the Gemini takeout images), pasted/saved images, diagram artifacts. → relates to G2 (media types), the transclusion layer, M5c surfaces. | 🔲 |
