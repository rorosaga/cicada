...
async def _run_engine_independent_tail(
    ...
    never sweep a projection dirtied by ``GET /state`` into a poll commit.
    Anything the polls or the question refresh change afterwards leaves the
    file one read behind (R2, disclosed).
    """
            ...
            if any(line[3:].strip() == state_dictionary.STATE_FILENAME for line in porcelain.splitlines()):
                ...
                async with _lock:
                    await git_service.commit_paths(
                        memory_path, state_message, [state_dictionary.STATE_FILENAME]
                    )
        except Exception as exc:
            logger.warning(
