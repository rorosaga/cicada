...
@pytest.mark.parametrize("tool", ["cicada_project", "cicada_note_progress", "cicada_backlog",
                                  "cicada_add_backlog_item", "cicada_add_backlog_note"])
@pytest.mark.parametrize("scopes", SUBSETS, ids=lambda s: "+".join(sorted(s)))
def test_the_project_sentences_appear_only_with_their_tool(scopes, tool):
    ...
    named in a remote primer only when the connection holds them (R12), and
    so are G150's three backlog tools."""
